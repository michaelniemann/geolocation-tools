...
# text_directory = os.path.normpath("full_texts/")
...
def map(filename, locations):
    ...
    if not all(math.isinf(coord) for coord in [minx, miny, maxx, maxy]) and not all(
        math.isnan(coord) for coord in [minx, miny, maxx, maxy]
    ):
        MAP_PADDING = 5
...
def textcheck(filename):
    ...
    plusfile = [
        xs + (text_filename,) for xs in entities
    ]  # this adds the FILEName into the CSV file

    # print(locations)

    with open(
        os.path.normpath(os.path.join(csv_directory, filename + ".csv")), "w"
    ) as f:  # Writing these to a CSV file that has the same name as the text file
